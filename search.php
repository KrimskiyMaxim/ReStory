...
<body>
	...
	<div id="content">
		<div id="posts">
		<?
			$post = getSearchArticles($_GET['source'], 'title', 0, 12);
			$numRecords = numSearchArticles($_GET['source'], 'title');
			if (!isset($post[0]['id'])) {
			...
			if($_GET['page'] > 1 && $_GET['page'] <= $numPages) {
				$i = $_GET['page'];
				$start = 12 * $i - 12;
				$post = getSearchArticles($_GET['source'], 'title', $start, 12);
			}}
		...
		<div class="page">
			...
			if (isset($post[0]['id'])) {
				if ($numPages > 1) {
					...
					echo('<a href="?source='.$_GET['source'].'&page='.$prewPage.'"<i class="fa fa-chevron-circle-left" aria-hidden="true"></i></a>');
					if(isset($_GET['page'])){echo($_GET['page']);} else {echo('1');}
					echo('<a href="?source='.$_GET['source'].'&page='.$nextPage.'"<i class="fa fa-chevron-circle-right" aria-hidden="true"></i></a>');
				}
			}
				?>
		</div>
